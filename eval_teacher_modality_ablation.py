...
def _load_model(config, ckpt_path, device, use_ema=True):
...
def parse_args():
    ...
    p.add_argument('--use_ema', type=lambda x: str(x).lower() in ('1', 'true', 'yes', 'y'), default=True)
...
def main():
    ...
    model = _load_model(config, ckpt_path, device, use_ema=args.use_ema)
    loss_fn = BCEDiceLoss(
