# -*- coding: utf-8 -*-
"""Qualitative visualization for student segmentation outputs (CT-only)."""
...
@torch.no_grad()
def save_student_diagnostics(task, loader, out_dir, num_samples=8, threshold=0.5):
    try:
        import matplotlib
        matplotlib.use('Agg')
        import matplotlib.pyplot as plt
    except ImportError:
        print('[vis_student] matplotlib not installed, skip diagnostics')
        return

    os.makedirs(out_dir, exist_ok=True)
    eval_model = task._get_eval_model()
    eval_model.eval()
    saved = 0
    ...
    if not hasattr(task, '_suppress_vis_log'):
        for v in task.networks.values():
            v.train()
    print(f'[vis_student] saved {saved} diagnostics to {out_dir}')
