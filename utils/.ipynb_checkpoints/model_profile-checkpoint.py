...
class TeacherSegWrapper(nn.Module):
    ...
    def forward(self, ct, pet):
        ...
        z_mri_g = self.encoder_general(h_mri)
        z_pet_g = self.encoder_general(h_pet)
        z_mri = self.encoder_mri(h_mri)
        z_pet = self.encoder_pet(h_pet)
...
def print_model_profile(stage_name, networks, config, is_teacher=True, image_size=None):
    ...
    try:
        ...
        if is_teacher:
            wrapper = TeacherSegWrapper(nets_copy, use_specific).to(device).eval()
