...
class MDTSegTeacher:
    ...
    def train_step(self, batch, forward_mode):
        ...
        ot_loss = aux_losses.get('pg_mtr_ot_loss', zero)
        if 'pg_mtr_ot_loss' in aux_losses and not torch.is_tensor(ot_loss):
            raise RuntimeError('PG-MTR OT loss must be a tensor.')
        ...
        ot_weight = float(getattr(self.config, 'pg_mtr_ot_weight', 0.01))
        if forward_mode == 'full':
            if 'pg_mtr_ot_loss' in aux_losses:
                if not torch.is_tensor(ot_loss):
                    raise RuntimeError('PG-MTR OT loss must be a tensor.')
                if not ot_loss.requires_grad:
                    raise RuntimeError('PG-MTR OT loss has no gradient.')
                if not torch.isfinite(ot_loss).all():
                    raise RuntimeError('PG-MTR OT loss contains NaN or Inf.')
                loss_total = loss_seg + ot_weight * ot_loss
            else:
                if route_weight > 0:
        ...
        loss_dict.update({'loss_seg': loss_seg.detach(), 'loss_total': loss_total.detach(), 'loss_full': loss_seg.detach() if forward_mode == 'full' else zero, 'loss_missing': loss_seg.detach() if forward_mode == 'missing' else zero, 'train_route_full': 1.0 if forward_mode == 'full' else 0.0, 'train_route_missing': 1.0 if forward_mode == 'missing' else 0.0, 'loss_pg_mtr_route': route_loss.detach(), 'loss_pg_mtr_mem': mem_loss.detach(), 'loss_pg_mtr_ot': ot_loss.detach(), 'weighted_loss_pg_mtr_route': (route_weight * route_loss).detach(), 'weighted_loss_pg_mtr_mem': (mem_weight * mem_loss).detach(), 'weighted_loss_pg_mtr_ot': (ot_weight * ot_loss).detach()})
