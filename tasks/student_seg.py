# -*- coding: utf-8 -*-
"""Training task for single-modality student (CT-only, no distillation).

Uses the same BCEDiceLoss as the teacher to ensure fair comparison.
Distillation losses will be added in a separate KD training script.
"""
...
def _forward(nets, ct, target_size):
    return nets['model'](ct, target_size=target_size)


class StudentSegTask:
    """Single-modality student segmentation training task."""
    ...
    def train_step(self, batch):
        ...
        outputs = _forward(self.networks, ct, mask.shape[-2:])
    ...
    def evaluate(self, loader, threshold=None, use_ema=True):
        ...
        for batch in loader:
            ct = batch['ct'].float().to(self.device)
            mask = batch['mask'].float().to(self.device)
            outputs = eval_model(ct, target_size=mask.shape[-2:])
            pred = self._select_main_pred(outputs)
