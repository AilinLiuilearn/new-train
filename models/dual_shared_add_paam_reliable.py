...
from models.paam_reliable_affine_memory import PETReliableAffineActionMemory


class DualSharedAddPAAMReliablePETCT(DualSharedAddPETCTBaseline):
    def __init__(self, *args, paam_k=8, **kwargs):
        super().__init__(*args, **kwargs)
        self.paam_k = int(paam_k)
        paam_channels = list(self.enc_pet.feature_info.channels())
        self.paam = PETReliableAffineActionMemory(channels=paam_channels, K=self.paam_k)
        paam_params = sum(p.numel() for p in self.paam.parameters())
        print(f'[dual_shared_add_paam_reliable] fusion=PAAM-Reliable paam_k={self.paam_k} paam_params={paam_params}')
    ...
    def _forward_full(self, ct, pet, target_size, capture_paam_visuals=False):
        return self._forward_paam(ct, pet, target_size, 'full', self.training, capture_paam_visuals)

    def _forward_missing(self, ct, pet, target_size, capture_paam_visuals=False):
        if self.training:
            return self._forward_paam(ct, pet, target_size, 'missing', True, capture_paam_visuals)
        return self._forward_paam(ct, None, target_size, 'missing', False, capture_paam_visuals)
    ...
    def _forward_auto(self, ct, pet, pet_available, target_size, capture_paam_visuals=False):
        ...
        if torch.all(pet_available == 1):
            return self._forward_full(ct, pet, target_size, capture_paam_visuals)
        if torch.all(pet_available == 0):
            return self._forward_missing(ct, pet, target_size, capture_paam_visuals)
        raise NotImplementedError('PAAM-Reliable only supports homogeneous batch state in auto mode')
    ...
    def forward(self, ct, pet=None, pet_available=None, target_size=None, forward_mode='auto', capture_paam_visuals=False):
        ...
        if forward_mode == 'auto':
            return self._forward_auto(ct, pet, pet_available, target_size, capture_paam_visuals)
