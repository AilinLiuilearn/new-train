...
def build_mdt_seg_teacher(config):
    ...
    if model_arch == 'dual_shared_add_baseline':
        ...
        model = DualSharedAddPETCTBaseline(
            ...
            ac_hmtr_num_tokens=getattr(config, 'ac_hmtr_num_tokens', 8),
            ac_hmtr_residual_scale_init=getattr(config, 'ac_hmtr_residual_scale_init', 0.1),
        )
        print(f'[dual_shared_add_baseline] ct={getattr(config, "ct_backbone", "convnextv2_nano")} pet={getattr(config, "pet_backbone", "mit_b1")} fusion=add shared_decoder=UNetStyleDecoder deep_supervision={_resolve_use_deep_supervision(config)} missing_mode={getattr(config, "missing_mode", "ct")}')
