...
class CTConditionedPETBenefitDistributionMemory(nn.Module):
    ...
    def collect_from_pair(
        ...
        for bi in range(b):
            ...
            stat_idx = deterministic_take(
                all_idx,
                min(self.max_stat_samples_per_image, h * w),
            )
        ...
        self._trim_synced_caches(
            [self._stat_q, self._stat_event, self._stat_delta, self._stat_benefit],
    ...
    def finalize_memory(self) -> Dict[str, Any]:
        ...
        values = {name: torch.zeros(self.K) for name in [
            ...
            "mu_neg", "sigma_neg", "count", "zero_count", "pos_count", "neg_count"
        ]}
        ...
        for k in range(self.K):
            ...
            values["pi_zero"][k] = zero.float().mean()
            values["pi_pos"][k] = pos.float().mean()
            values["pi_neg"][k] = neg.float().mean()
            if pos.any():
                values["mu_pos"][k] = d[pos].mean()
                values["sigma_pos"][k] = d[pos].std(unbiased=False)
            if neg.any():
                values["mu_neg"][k] = d[neg].mean()
                values["sigma_neg"][k] = d[neg].std(unbiased=False)
    ...
    def retrieve(
        ...
        query, q_aux = self.query_builder(ct_decoder_feature, ct_logits)
        b, _, h, w = query.shape
        ct_prob = q_aux["probability"]

        if not bool(self.memory_ready.item()):
            zero = torch.zeros_like(ct_prob)
            info = {
                "memory_ready": torch.tensor(False, device=ct_logits.device),
                "entropy": zero,
                "effective_slots": torch.ones_like(zero),
                "top1_weight": zero,
                "top1_slot": torch.zeros_like(zero, dtype=torch.long),
                "max_similarity": zero,
                "weights": torch.zeros(b, self.K, h, w, device=ct_logits.device),
                "slot_expected_delta": self.slot_expected_delta(),
            }
            return RetrievalResult(zero, ct_prob, ct_logits, info)

        q = F.normalize(query.permute(0, 2, 3, 1).reshape(-1, self.query_dim), dim=1, eps=1e-6)
        ...
        delta_flat = weights @ self.slot_expected_delta()
        delta = delta_flat.reshape(b, 1, h, w)
        corrected_prob = (ct_prob + delta).clamp(1e-5, 1.0 - 1e-5)
        corrected_logits = torch.logit(corrected_prob)
        ...
        weights_map = weights.reshape(b, h, w, self.K).permute(0, 3, 1, 2)

        with torch.no_grad():
            n = int(entropy.numel())
            for name, value in [
                ("entropy", entropy.mean()),
                ("effective_slots", effective.mean()),
        ...
        info = {
            "memory_ready": torch.tensor(True, device=ct_logits.device),
            "entropy": entropy_map,
            "effective_slots": effective_map,
            "top1_weight": top1_weight_map,
            "top1_slot": top1_slot_map,
            "max_similarity": max_similarity_map,
            "weights": weights_map,
            "slot_expected_delta": self.slot_expected_delta(),
            "logit_scale": scale,
        }
    ...
    # ------------------------------ 单张组合图 ------------------------------
    @staticmethod
    def _render_map(array: np.ndarray, title: str, vmin=None, vmax=None) -> Image.Image:
    ...
    @staticmethod
    def _render_bar(values: Sequence[float], title: str, ylabel: str) -> Image.Image:
        fig = plt.figure(figsize=(3.1, 2.9), dpi=120)
    ...
    def save_composite_visualization(
        ...
        full_probability: Optional[torch.Tensor] = None,
        benefit: Optional[torch.Tensor] = None,
        ...
        m = self._last_maps
        zeros = np.zeros_like(m["ct_probability"].numpy())
        panels = [
            self._render_map(m["ct_probability"].numpy(), "Masked CT-only probability", 0, 1),
            self._render_map(m["corrected_probability"].numpy(), "CPBDM corrected probability", 0, 1),
            self._render_map(m["delta_probability"].numpy(), "Retrieved probability correction", -1, 1),
            self._render_map(m["ct_entropy"].numpy(), "CT prediction entropy"),
            self._render_map(m["ct_boundary"].numpy(), "CT probability boundary"),
            self._render_map(m["retrieval_entropy"].numpy(), "Retrieval entropy"),
            self._render_map(m["effective_slots"].numpy(), "Effective retrieved slots"),
            self._render_map(m["top1_slot"].numpy(), "Top-1 memory slot"),
            self._render_map(target[0, 0].cpu().numpy() if target is not None else zeros, "Ground truth", 0, 1),
            self._render_map(full_probability[0, 0].cpu().numpy() if full_probability is not None else zeros, "Full CT+PET probability", 0, 1),
            self._render_map(benefit[0, 0].cpu().numpy() if benefit is not None else zeros, "Full-vs-CT local benefit"),
            self._render_map(event[0, 0].cpu().numpy() if event is not None else zeros, "Event map: -1 / 0 / +1", -1, 1),
        ]
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        path = output_dir / f"{tag}_cpbdm_composite.png"
        self._compose(panels, rows=3, cols=4, path=path)
