...
新增研究超参数只有两个：
- ``filter_ratio=0.05``：Full batch 原型更新时丢弃的离群样本比例；
- ``reference_weight=0.1``：Missing batch 语义参考辅助损失权重。
"""
...
class _ScaleReferenceBank(nn.Module):
    ...
    def __init__(
        ...
        # count 统计被中心过滤后接纳的样本/切片级向量数，
        # 使用累计均值避免引入 EMA 动量超参数。
        self.register_buffer("counts", torch.zeros(self.NUM_CLASSES, dtype=torch.float64))
    ...
    @staticmethod
    def _sample_level_vectors(
        feature: Tensor,
        scaled_target: Tensor,
        class_index: int,
    ) -> Tensor:
        """对每个含该类别的2D样本分别做mask pooling，避免按像素量加权。"""
        vectors: List[Tensor] = []
        for batch_index in range(feature.shape[0]):
            class_mask = scaled_target[batch_index] == class_index
            if bool(class_mask.any()):
    ...
    def _centroid_filter(
        ...
        """按配对CT-PET向量到联合中心的余弦偏离过滤离群样本。"""
        if ct_vectors.shape != pet_vectors.shape:
            raise RuntimeError("CT/PET样本级候选向量必须一一配对且维度相同。")
        num_samples = ct_vectors.shape[0]
        if num_samples <= 1 or self.filter_ratio == 0.0:
            return ct_vectors, pet_vectors

        # 将目标过滤比例转换为最接近的整数样本数。
        # 例如 N=16、rho=0.05 时 round(0.8)=1，因此过滤1个；
        # N=2 时 round(0.1)=0，不对小集合强制删除一个样本。
        # min(..., N-1) 保证无论过滤率多大都至少保留一个参考样本。
        drop_count = min(
            num_samples - 1,
            int(round(num_samples * self.filter_ratio)),
        )
        if drop_count == 0:
            return ct_vectors, pet_vectors
        ...
        centroid = F.normalize(joint.mean(dim=0, keepdim=True), dim=1)
        distance = 1.0 - F.cosine_similarity(joint, centroid.expand_as(joint), dim=1)
        keep_count = num_samples - drop_count
        keep_indices = torch.topk(distance, k=keep_count, largest=False).indices
        return ct_vectors[keep_indices], pet_vectors[keep_indices]
    ...
    def update(self, ct_feature: Tensor, pet_feature: Tensor, target: Tensor) -> None:
        ...
        scaled_target = _resize_target(target, ct_feature.shape[-2:])
        for class_index in range(self.NUM_CLASSES):
            ct_vectors = self._sample_level_vectors(
                ct_feature.detach(), scaled_target, class_index
            )
            pet_vectors = self._sample_level_vectors(
                pet_feature.detach(), scaled_target, class_index
            )
...
class MTPICompensation(nn.Module):
    ...
    channels:
        四个尺度在现有对齐层之后的共同 CT/PET 通道数。
    filter_ratio:
        Full原型更新时过滤的离群样本/切片比例，默认0.05。
    sync_distributed:
        DDP训练时是否跨进程同步样本/切片级原型统计。它是执行选项而非研究超参数。
    """
    ...
    def reference_loss(
        ...
        """唯一辅助损失：补偿特征的样本级类别语义应接近对应PET原型。

        只对当前已经建立参考原型且该样本真实存在的类别计算余弦分类损失。
        它不访问当前样本的真实PET，也不做逐元素PET特征回归。
        """
        features = self._validate_features(
            compensated_pet_features, "compensated_pet_features"
        )
        ...
        for feature, bank in zip(features, self.reference_banks):
            ...
            pooled_labels: List[int] = []
            for class_index, logit_index in class_to_logit.items():
                class_vectors = _ScaleReferenceBank._sample_level_vectors(
                    feature, scaled_target, class_index
                )
                if class_vectors.shape[0] > 0:
                    pooled_vectors.append(class_vectors)
...
def _smoke_test() -> None:
    ...
    # 中心过滤整数化规则检查：N=16时过滤1个，N=2时不强制过滤。
    bank = module.reference_banks[0]
    filter_ct = torch.randn(16, channels[0])
    filter_pet = torch.randn(16, channels[0])
    kept_ct, kept_pet = bank._centroid_filter(filter_ct, filter_pet)
    assert kept_ct.shape[0] == 15 and kept_pet.shape[0] == 15
    small_ct = torch.randn(2, channels[0])
    small_pet = torch.randn(2, channels[0])
    kept_ct, kept_pet = bank._centroid_filter(small_ct, small_pet)
    assert kept_ct.shape[0] == 2 and kept_pet.shape[0] == 2
