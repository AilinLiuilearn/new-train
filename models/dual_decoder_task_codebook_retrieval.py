...
from models.task_driven_codebook import TaskDrivenLearnableCodebook
...
class DualDecoderTaskCodebookRetrieval(nn.Module):
    def __init__(self, ct_backbone='convnextv2_nano', pet_backbone='mit_b1', ct_pretrained_path=None, pet_pretrained_path=None, in_channels=3, out_channels=1, decoder_channels=(512, 256, 128, 64), use_deep_supervision=False, task_codebook_stages='all', task_codebook_num_tokens=8, task_codebook_temperature=0.07):
        super().__init__()
        ...
        self.task_codebook = TaskDrivenLearnableCodebook(pet_channels, num_tokens=task_codebook_num_tokens, temperature=task_codebook_temperature, stage_mode=task_codebook_stages)
        self.retrieval_adapters = nn.ModuleDict()
        for s in self.task_codebook.active_stage_numbers:
            ch = pet_channels[s - 1]
            adapter = nn.Sequential(nn.Conv2d(self.task_codebook.latent_dim, ch, 1, bias=False), nn.GroupNorm(_valid_group_count(ch), ch, affine=False))
            nn.init.kaiming_normal_(adapter[0].weight, mode='fan_out', nonlinearity='linear')
            self.retrieval_adapters[str(s)] = adapter
            self.register_parameter(f'gamma_s{s}', nn.Parameter(torch.tensor(0.01).log()))
    ...
    def _forward_full(self, ct, pet, target_size):
        ...
        petf = self._encode_pet(pet)
        out = self._decode_with(self.full_decoder, self.fusion(aligned, petf, pet_available=None), target_size)
        out['aux_losses'] = {}
        out['diagnostics'] = {}
        return out

    def _apply_retrieved_memory(self, aligned, retrieved, diagnostics):
        missing = list(aligned)
        out = dict(diagnostics)
        for s in self.task_codebook.active_stage_numbers:
            i = s - 1
            feat = self.retrieval_adapters[str(s)](retrieved[s])
            alpha = torch.nn.functional.softplus(getattr(self, f'gamma_s{s}'))
            delta = alpha * feat
            missing[i] = aligned[i] + delta
            out[f'task_codebook_s{s}_gamma'] = alpha.detach()
            out[f'task_codebook_s{s}_ct_rms'] = self._rms_tensor(aligned[i])
            out[f'task_codebook_s{s}_injection_rms'] = self._rms_tensor(delta)
            out[f'task_codebook_s{s}_injection_ct_ratio'] = out[f'task_codebook_s{s}_injection_rms'] / (out[f'task_codebook_s{s}_ct_rms'] + 1e-6)
        return missing, out

    def _forward_missing(self, ct, target_size):
        aligned = self._encode_ct(ct)
        retrieved, diagnostics = self.task_codebook(aligned)
        missing, diagnostics = self._apply_retrieved_memory(aligned, retrieved, diagnostics)
        out = self._decode_with(self.missing_decoder, missing, target_size)
        out['aux_losses'] = {}
    ...
    def _forward_auto(self, ct, pet, pet_available, target_size):
        ...
        if full_idx.numel() > 0:
            ...
            full_outputs = self._decode_with(self.full_decoder, self.fusion(full_ct, self._encode_pet(pet_full), pet_available=None), target_size)
            full_outputs['aux_losses'] = {}
            full_outputs['diagnostics'] = {}
        if missing_idx.numel() > 0:
            miss_ct = [f.index_select(0, missing_idx) for f in aligned]
            retrieved, diagnostics = self.task_codebook(miss_ct)
            miss_feat, diagnostics = self._apply_retrieved_memory(miss_ct, retrieved, diagnostics)
            missing_outputs = self._decode_with(self.missing_decoder, miss_feat, target_size)
            missing_outputs['aux_losses'] = {}
            missing_outputs['diagnostics'] = diagnostics
