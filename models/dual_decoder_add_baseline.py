...
class DualDecoderAddPETCTBaseline(nn.Module):
    def __init__(
        self,
        ...
        self.full_decoder = UNetStyleDecoder(
            pet_channels,
    ...
    def _forward_full(self, ct, pet, target_size):
        ...
        return self._decode_with(self.full_decoder, fused_feats, target_size)

    def _forward_missing(self, ct, target_size):
        aligned_ct_feats = self._encode_ct(ct)
        return self._decode_with(self.missing_decoder, aligned_ct_feats, target_size)

    @staticmethod
    def _merge_output_tensors(base_tensor, update_tensor, indices):
        merged = base_tensor.clone()
        return merged.index_copy(0, indices, update_tensor)
    ...
    def _forward_auto(self, ct, pet, pet_available, target_size):
        ...
        if missing_idx.numel() > 0:
            ...
            missing_outputs = self._decode_with(self.missing_decoder, missing_ct_feats, target_size)
