...
def _load_state_dict_with_report(model, checkpoint_path, expect_joint=False):
    checkpoint = torch.load(checkpoint_path, map_location='cpu')
    state_dict = checkpoint.get('model', checkpoint)
    if expect_joint:
        sample_keys = list(state_dict.keys())[:8] if isinstance(state_dict, dict) else []
        looks_joint = any(
            k.startswith('stage1.') or k.startswith('role_fusion.')
            for k in state_dict.keys()
        )
        has_adapters = any(k.startswith('decoder_adapters.') for k in state_dict.keys())
        looks_stage1_only = any(k.startswith('enc_ct.') for k in state_dict.keys()) and not looks_joint
        if looks_stage1_only or not looks_joint:
            raise RuntimeError(
                'resume_checkpoint does not look like a joint Recovery+SPRE model. '
                f'path={checkpoint_path} sample_keys={sample_keys}'
            )
        if has_adapters:
            print(
                '[RESUME][WARN] checkpoint contains legacy decoder_adapters.*; '
                'current joint model no longer uses adapters',
                flush=True,
            )
    ...
    if expect_joint:
        critical_missing = [
            ...
            if k.startswith('role_fusion.') or k.startswith('stage1.decoder.') or k.startswith('stage1.enc_')
        ]
        if critical_missing:
            raise RuntimeError(f'Joint resume missing critical keys: {critical_missing[:20]}')
    return checkpoint
...
def _log_stage2_stats(outputs, route, batch_idx):
    ...
    if not isinstance(stats, dict) or not stats:
        return
    parts = [f'[SPRE STATS] batch={batch_idx + 1} route={route}']
...
def main():
    ...
    is_joint = str(getattr(cfg, 'model_arch', '')).strip() == 'prompt_role_expert_stage2'
    print(f"[INFO] starting {'joint Recovery+SPRE' if is_joint else 'baseline'} training", flush=True)
    ...
    if getattr(cfg, 'resume_checkpoint', None):
        # NOTE: model weights only; optimizer/scheduler/scaler are not restored here.
        _load_state_dict_with_report(task.model, cfg.resume_checkpoint, expect_joint=is_joint)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            # Classic 1:1 alternating Full / Missing (same as original Stage-1).
        ...
        print(
            ...
            f'bank_version={extra_metrics["cppi_bank_version"]}',
            flush=True,
        )
