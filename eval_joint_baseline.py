...
def main():
    ...
    ckpt_path = os.path.join(args.checkpoint_dir, 'ckpt.best_joint.pth.tar')
    ...
    # Full-checkpoint recovery must not depend on Stage-1 init or external
    # text cache: structure comes from saved config + archived module2 state.
    saved_config['stage1_init_enabled'] = False
    saved_config['stage1_ct_checkpoint'] = None
    saved_config['stage1_pet_checkpoint'] = None
    cfg = SegMDTConfig(args=saved_config)

    task = MDTSegTeacher(build_mdt_seg_teacher(cfg, model_state_dict=ckpt['model']), cfg)
